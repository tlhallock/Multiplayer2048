package org.hallock.tfe.cmn.svr;
...
import org.hallock.tfe.serve.InGamePlayer;
...
public class EvilActionsAwarder
{
	...
	{
		if (Constants.random.nextDouble() >= awardPercent)
		{
			return null;
		}

		DiscreteDistribution ceilingEntry;
		Entry<BigDecimal, DiscreteDistribution> ceilingEntry2 = distributions.ceilingEntry(player.points.getPoints());
		...
		{
		case AddHighTile:
			return new AddHighTile(player.board.getHighestTile() + 2);
		case BlockCell:
			return new BlockCell();
		case AddInMoreTiles:
			return new AddInMoreTiles(
					new DiscreteDistribution(new int[] {1}, new double[]{1}),
					(int) (.3 * Constants.random.nextDouble() * player.board.getNumCells()));
		case AddRandomMove:
		...
		case RemovePoints:
			return new RemovePoints(
					multiplyIntegers(
							...
							new BigDecimal(String.format("%17.17f", Constants.random.nextDouble() / 3))));
		default:
