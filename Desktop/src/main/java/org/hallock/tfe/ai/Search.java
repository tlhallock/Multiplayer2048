...
public class Search<T extends Comparable<T>>
{
	...
	int desiredBreadth = 50;
	private int desiredDepth = 20;
	
	Search(Heuristic<T> h)
	{
		this.h = h;
	}
		...
		public void run()
		{
			...
			try
			{
				while (tId == transactionId)
				{
					...
					if (take == null || take.event.depth >= desiredDepth || take.event.computed.size() >= desiredBreadth || take.event.isFinished())
					{
						continue;
					}
...
//			}
	...
	public class EventSearchTreeNode extends SearchTreeNode
	{
		...
		MedianSet<T, DecisionSearchTreeNode> computed = new MedianSet<>();
		
		public EventSearchTreeNode(TileBoard newState, DecisionSearchTreeNode parent, int depth, SearchRunnable state)
		{
			super(newState, depth, parent, state);
			possibles = newState.getPossibles();
		...
		{
			return alreadyComputed != null || possibles.isEmpty();
		}
		...
		public void sample(int size)
		{
			...
			MedianSet<T, DecisionSearchTreeNode>.Pair oldPair;
			...
			synchronized (this)
			{
				while (size --> 0 && !possibles.isEmpty())
				{
					...
					newBoard.tiles[removeFirst.x][removeFirst.y] = 1;
					DecisionSearchTreeNode dec = new DecisionSearchTreeNode(newBoard, this, depth + 1, state);

					synchronized (computed)
					{
						computed.add(dec.getComparator(), dec);
					}
				}

				MedianSet<T, Search<T>.DecisionSearchTreeNode>.Pair center = computed.getCenter();
		...
		{
			if (computed.changed(decisionSearchTreeNode, decisionSearchTreeNode.getComparator()))
				valueChanged();
		}
	...
	public class DecisionSearchTreeNode extends SearchTreeNode
	{
		...
		public DecisionSearchTreeNode(TileBoard newState,  EventSearchTreeNode parent, int depth, SearchRunnable state)
		{
			super(newState, depth, parent, state);
