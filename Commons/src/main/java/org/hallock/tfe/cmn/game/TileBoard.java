...
//	}
	...
	public void randomlyFill(int num, DiscreteDistribution dist, TileChanges changes)
	{
		...
		Collections.shuffle(possibles, GameConstants.random);
	...
	{
		StringBuilder builder = new StringBuilder(tiles.length * (1 + (GameConstants.DISPLAY_WIDTH + 1) * tiles[0].length));

		for (int i = 0; i < tiles.length; i++)
		{
			for (int j = 0; j < tiles[i].length; j++)
			{
				builder.append(Utils.ensureLength(Utils.display(tiles[i][j]), ' ', GameConstants.DISPLAY_WIDTH)).append(' ');
			}
			builder.append('\n');
		}
	...
	public void print(PrintStream ps, int depth)
	{
		...
		for (int i = 0; i < tiles.length; i++)
		{
			...
			for (int j = 0; j < tiles[i].length; j++)
			{
				ps.append(Utils.ensureLength(Utils.display(tiles[i][j]), ' ', GameConstants.DISPLAY_WIDTH)).append(' ');
			}
			ps.append('\n');
		}
	}
